classdef LfpApp < handle
    ...
    methods (Access=private)
        ...
        function openImportDialog(app, filename)
            ...
            importButton = uibutton(buttons, 'Text', '确认导入', 'ButtonPushedFcn', @(s,e)app.finishImport(dialog, filename, struct('delimiter', delimiter, 'header', header, 'dataStart', dataStart, 'timeColumn', timeColumn, 'signals', signals, 'direction', direction, 'fs', fs, 'units', units, 'scale', scale, 'timeUnit', timeUnit, 'status', status)));
        ...
        function finishImport(app, dialog, filename, controls)
            try
                ...
                [data, ~] = lfp_import_csv_configured(filename, Delimiter=delimiter, HeaderRow=header, ...
                    ...
                    TimeUnit=string(controls.timeUnit.Value));
        ...
        function updateDataInfo(app)
            ...
            app.Controls.InfoArea.Value = { ...
                "文件：" + string(d.metadata.sourceFileName), ...
                "IPG SN：" + ipg, ...
                sprintf('通道数：%d | 样本数：%d', c, n), ...
                sprintf('采样率：%.6g Hz | 时长：%.3f s', d.fs, duration), ...
                "单位：" + string(d.units), sprintf('NaN/Inf：%d', missing)};
        end
